...
const ListScreen = ({ data, navigation }) => {
    ...
    return (
        <>
            {data?.map((item) => {
                return (
                    <TouchableOpacity activeOpacity={0.6} onPress={()=>onPress(item?.navigation)} style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10, marginVertical: 8, borderRadius: 10, backgroundColor: item?.name == "Connect with Me" && COLORS.Primary_2 }}>
